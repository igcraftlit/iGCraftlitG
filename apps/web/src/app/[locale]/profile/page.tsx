/**
 * 文件：apps/web/src/app/[locale]/profile/page.tsx
 * 所属层：前端（App Router 页面）
 * 路由：G_User（个人主页）
 * 模块：iGM_ProfilePage
 * 作用：个人主页模块一占位页，仅展示页面标题与建设中说明
...
export const generateMetadata = async ({ params }: { params: Promise<{ locale: iGM_Locale }> }) =>
  iGM_pageMetadata((await params).locale, 'G_User');

// ==================== 区块：页面 ====================
export default async function iGM_ProfilePage({
  params,
}: {
  ...
  return <IGM_PagePlaceholder title={t('G_User')} />;
}
