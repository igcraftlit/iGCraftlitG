/**
 * 文件：apps/web/src/app/[locale]/activities/page.tsx
 * 所属层：前端（App Router 页面）
 * 路由：G_Activity（活动）
 * 模块：iGM_ActivityPage
 * 作用：活动模块一占位页，仅展示页面标题与建设中说明
...
export const generateMetadata = async ({ params }: { params: Promise<{ locale: iGM_Locale }> }) =>
  iGM_pageMetadata((await params).locale, 'G_Activity');

// ==================== 区块：页面 ====================
export default async function iGM_ActivityPage({
  params,
}: {
  ...
  return <IGM_PagePlaceholder title={t('G_Activity')} />;
}
