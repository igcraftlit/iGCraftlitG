/**
 * 文件：apps/web/src/app/[locale]/posts/page.tsx
 * 所属层：前端（App Router 页面）
 * 路由：G_Post（帖子）
 * 模块：iGM_PostListPage
 * 作用：帖子列表模块一占位页，仅展示页面标题与建设中说明
...
export const generateMetadata = async ({ params }: { params: Promise<{ locale: iGM_Locale }> }) =>
  iGM_pageMetadata((await params).locale, 'G_Post');

// ==================== 区块：页面 ====================
export default async function iGM_PostListPage({
  params,
}: {
  ...
  return <IGM_PagePlaceholder title={t('G_Post')} />;
}
