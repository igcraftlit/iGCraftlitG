/**
 * 文件：apps/web/src/app/[locale]/community/page.tsx
 * 所属层：前端（App Router 页面）
 * 路由：G_Community（社区广场）
 * 模块：iGM_CommunityPage
 * 作用：社区广场模块一占位页，仅展示页面标题与建设中说明
...
export const generateMetadata = async ({ params }: { params: Promise<{ locale: iGM_Locale }> }) =>
  iGM_pageMetadata((await params).locale, 'G_Community');

// ==================== 区块：页面 ====================
export default async function iGM_CommunityPage({
  params,
}: {
  ...
  return <IGM_PagePlaceholder title={t('G_Community')} />;
}
